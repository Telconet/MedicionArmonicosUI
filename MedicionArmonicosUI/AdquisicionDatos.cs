...
namespace MedicionArmonicosUI
{
    class AdquisicionDatos
    {
        ...
        public AdquisicionDatos(int[] canalInferior, int[] canalSuperior, int[] frecuenciaMuestro, int ventanaMuestro, bool almacenarHoraMedicion, String ruta, float divisorVoltaje, float vueltasTransformador, bool medicionContinua, int numeroTarjetas)
        {
        ...
        private void iniciarAdquisicionDatosFn()
        {
            ...
            if (!iniciado)
            {
                if (!this.medicionContinua)
                {
                    ...
                    for (int i = 0; i < numeroTarjetas; i++)
                    {
                        this.almacenador[i] = new AlmacenadorLecturas(config, "tarjeta_" + i.ToString());
